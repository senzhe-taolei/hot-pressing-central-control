...
class ChengLiao:
    ...
    def init(self):
        # 初始时现将所有状态清零
        self.main_master_right.write_single_register(chengliao_write_action_status_addr, 0)
        self.auxiliary_master_right.write_single_register(chengliao_write_action_status_addr, 0)
        # 称重联动on开启
        addr = chengliao_write_sites["称重联动on"]["Address"]
        self.main_master_right.set_db_bool(addr, 1)
        self.auxiliary_master_right.set_db_bool(addr, 1)


    def set_params(self, params):
        # todo 值判断只判断一次，应该由上层中控服务器判断好，此处不应该再判断
        # 主料投料量  辅料投料量
    ...
    def chengliao_loop_read_all_sites(self):
        while True:
            if conf.left_or_right == "left":
                left_right = {
                    "main:l": self.main_master_left, "auxiliary:l": self.auxiliary_master_left,
                }
            elif conf.left_or_right == "right":
                left_right = {
                    "main:r": self.main_master_right, "auxiliary:r": self.auxiliary_master_right,
                }
            else:
                raise
...
if __name__ == "__main__":
    # 监听称料
    ChengLiao().chengliao_loop_read_all_sites()
